...
public class Main {
    ...
    public static void ReadInput() {
        try {
            ...
            while ((S = InFile.readLine()) != null) {
                // If this flag is seen, the next line contains the minimum
                // number of posts that a thread can have since the last scan to
                // be considered active
                if (S.equals("[MINTHREADPOSTS]")) {
                    S = InFile.readLine().trim();
                    MinThreadPosts = Integer.parseInt(S);
                } // If this flag is seen, the next line contains the username
                  // of the account from which automated PM's are sent
                else if (S.equals("[AUTOPMUSER]")) {
                    S = InFile.readLine().trim();
                    AutoPM.setAutoPMUser(S);
                } // If this flag is seen, the next line contains the password
                  // of the account from which automated PM's are sent
                else if (S.equals("[AUTOPMPASS]")) {
                    S = InFile.readLine().trim();
                    AutoPM.setAutoPMPass(S);
                } // If this flag is seen, the next few lines contain the
                  // default message to send in an automatic PM
                  ...
                  // default message for the first activity strike
                  ...
                  // default form for a stalled thread
                  ...
                  // default message to send when a strike is removed for good
                  ...
                  // default message for expired hiatus warnings
                ...
                } // If this flag is seen, the next few lines contian the
                  // default mssage to send when a user makes a post htat is
                  // too short
                  ...
                  // default form for posts which were too short
                  ...
                  // default message to send when a user makes a post tha tis
                  // too long
                ...
                } // If this flag is seen, the next few lines contain the form
                  // for a post which was too long
                else if (S.equals("[LONGPOSTMESSAGE]")) {
                    S = InFile.readLine();
                    String message = "";
                    while (!S.equals("[ENDMESSAGE]")) {
            ...
            while ((S = InFile.readLine()) != null) {
                ...
                // Read in the time of the last post in minutes since Jan 1,
                // 2009
                ...
                // Read in the name of the board associated with the last thread
                // the character posted in
    ...
    static void GetForums() {
        try {
            ...
            while (ForumMatcher.find()) {
                ProcessForum(
                        ForumMatcher.group(2),
                        "http://rp.mokou.org"
                                + ForumMatcher
                                        .group(1)
                                        .substring(ForumMatcher.group(1).indexOf("/"),
                                                ForumMatcher.group(1).indexOf("&amp;sid=")).replace("&amp;", "&"), true);
            }
        } catch (Exception e) {
    ...
    static void ProcessForum(String name, String URL, boolean active) {
        try {
            ...
            // Get any log forums, and add them to the list of places to be
            // crawled
            Pattern ForumPattern = Pattern.compile("<a href=\"([^\"]*)\" class=\"forumtitle\">([^<]*)</a>");
            Matcher ForumMatcher = ForumPattern.matcher(resource);
                ...
                ProcessForum(
                        ...
                        "http://rp.mokou.org"
                                + ForumMatcher
                                        .group(1)
                                        .substring(ForumMatcher.group(1).indexOf("/"),
                                                ForumMatcher.group(1).indexOf("&amp;sid=")).replace("&amp;", "&"),
                        false);
            }

            // Cut off global announcements from the top of the forum before
            // searching, if htere are no topics, then continue to the next
            // forum
            ...
            while (ThreadMatcher.find() && ThreadTimeMatcher.find()
                    && (active || Utilities.ParseDateString(ThreadTimeMatcher.group(1).trim()) > LastScanTime)) {
                ProcessThread(
                        name,
                        ThreadMatcher.group(2),
                        "http://rp.mokou.org"
                                + ThreadMatcher
                                        .group(1)
                                        .substring(ThreadMatcher.group(1).indexOf("/"),
                                                ThreadMatcher.group(1).indexOf("&amp;sid=")).replace("&amp;", "&"),
                        active);
            }
    ...
    static void ProcessThread(String parent, String name, String URL, boolean active) {
        try {
            // Start up a set of players to have their current threads
            // incremented
                ...
                while (PostMatcher.find() && ContentMatcher.find()) {
                    ...
                    if (Utilities.ParseDateString(PostMatcher.group(2).trim()) > LastScanTime) {
                        ...
                        if (GUI.playerTableModel.containsPlayer(PostMatcher.group(1).trim())) {
                            ...
                            if (wordCount < 100) {
                                if (GUI.playerTableModel.containsPlayer(PostMatcher.group(1).trim())) {
                                    GUI.playerTableModel.getPlayerByName(PostMatcher.group(1).trim()).addShortPost(
                                            new BadPost(parent, name, wordCount));
                                }
                                Placeholder.add(PostMatcher.group(1).trim() + " {" + parent + "} " + name + "\r\n");
                            } else {
                                Players.add(PostMatcher.group(1).trim());
                                GUI.playerTableModel.getPlayerByName(PostMatcher.group(1).trim()).updateLastPost(
                                        PostMatcher.group(2).trim(), name, parent);
                                // Tell the user that an overlimit post was
                                // detected
                                if (wordCount > 750) {
                                    if (GUI.playerTableModel.containsPlayer(PostMatcher.group(1).trim())) {
                                        GUI.playerTableModel.getPlayerByName(PostMatcher.group(1).trim()).addLongPost(
                                                new BadPost(parent, name, wordCount));
                                    }
                            ...
                            GUI.PrintLineToMainOutput("Unrecognized Player: " + PostMatcher.group(1).trim()
            ...
            // If this thread is inactive, add it to the table of stalled
            // threads
            ...
            // Increment thread counts for all players posting actively in this
            // thread
